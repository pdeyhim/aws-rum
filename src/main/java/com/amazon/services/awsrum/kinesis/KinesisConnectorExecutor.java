...
import com.amazon.services.awsrum.utils.DynamoDBUtils;
import com.amazon.services.awsrum.utils.KinesisUtils;
import com.amazon.services.awsrum.utils.RedshiftUtils;
import com.amazon.services.awsrum.utils.S3Utils;
...
public abstract class KinesisConnectorExecutor<T,U> extends KinesisConnectorExecutorBase<T,U> {
    ...
    private static final String DYNAMODB_RANGE_KEY = "dynamoDBRangeKey";
    ...
    private void setupAWSResources() {
        ...
        if (parseBoolean(CREATE_DYNAMODB_DATA_TABLE, DEFAULT_CREATE_RESOURCES, properties)) {
            ...
            String rangeKey = properties.getProperty(DYNAMODB_RANGE_KEY);
            ...
            if (rangeKey.length()>0){
            	createDynamoDBTable(key, rangeKey,readCapacityUnits, writeCapacityUnits);
            } else {
   ...
   /* private void setupInputStream() {
        if (parseBoolean(CREATE_STREAM_SOURCE, DEFAULT_CREATE_STREAM_SOURCE, properties)) {
            ...
            StreamSource streamSource = new StreamSource(config, inputFile);
    ...
    private static List<String> getKinesisMessageModelFields() {
		...
		map.put("String", "VARCHAR(max)");
		map.put("int", "INTEGER");
		map.put("Boolean", "BOOLEAN");
    	
    	List<String> fields = new ArrayList<String>();
	    Field[] allFields = KinesisMessageModel.class.getDeclaredFields();
	    for (Field field : allFields) {
	    	
	    	fields.add(field.getName()+" "+map.get(field.getType().getSimpleName()));
	    }
		    return fields;
    }
